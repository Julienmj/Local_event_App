<template>
  <div class="ai-panel">
    <div class="ai-panel-header">
      ...
      <span class="ai-panel-title">AI Copilot</span>
      <div class="ai-status">
...
const history = ref([
  { role: 'assistant', content: 'Hello! I\'m your EventLocal AI Copilot. Ask me to draft an agenda, estimate a budget, write marketing copy, or forecast attendance for your event.' }
])
const inputText = ref('')
  ...
  try {
    const res = await fetch(ANTHROPIC_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        messages: history.value.slice(-6).filter(m => m.role !== 'assistant' || history.value.indexOf(m) > 0),
      }),
    })
    const data = await res.json()
    if (!res.ok) throw new Error(data.error?.message || 'AI error')
    history.value.push({ role: 'assistant', content: data.content?.[0]?.text || 'No response.' })
  } catch (e) {
    history.value.push({ role: 'assistant', content: "I couldn't connect right now. Please try again." })
    show('AI Copilot unavailable — check your API key.', '⚠️')
  } finally {
    loading.value = false
    await scrollBottom()
  }
